...
- 古典概型 $P(A)=\frac{A\text{所包含的试验结果的总数}}{\text{试验结果的总数}}$
...
$A,B \text{独立} \Longleftrightarrow A,\overline B \text{独立} \Longleftrightarrow \overline A,B \text{独立} \Longleftrightarrow \overline A,\overline B \text{独立}$
...
随机变量独立 $F(x,y)=F_X(x)F_Y(y) \Longleftrightarrow f(x,y)=f_X(x)f_Y(y)\text{几乎处处成立} \Longleftrightarrow X,Y\text{独立} $
...
I 类错误概率 $\alpha(\mu)=P\{\text{拒绝}H_0|H_0\text{为真}\}$

II 类错误概率 $\beta(\mu)=P\{\text{接受}H_0|H_0\text{为假}\}$
